...
set.seed(123)
output = ancombc2(data = prok, tax_level = "Genus",
...
fig_type = df_fig_type_filtered %>%
  ...
  labs(y = "Prokaryotic Genus", x = "Log fold change", 
       title = NULL) + 
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        axis.text.y = element_text(angle = 0, hjust = 1,
                                   color = df_fig_type_filtered$color, size = 8))
...
fig_type +
  geom_text(
    aes(
      x = ifelse(seq_along(taxon) %in% 67:77, 0.2, 0),  # Shift only specified row(s) to the right
      y = taxon, 
      label = paste("p =", p_formatted_c),  # Add "p =" to each label
      hjust = ifelse(seq_along(taxon) %in% 67:77, 0, 1.2),  # Adjust hjust for specified row(s)
    ),
