...
#
# Description:
#   PCoA analysis of 16S rRNA gene diversity with elemental concentrations as
...
prok_abund_file_path <- "../18S/table-filtered.qza"
prok_taxonomy_file_path <- "../18S/taxonomy.qza"
prok_tree_data <- "../18S/rooted-tree.qza"
prok_rep_data <- "../18S/rep-seqs-filtered.qza"
...
abund_raw <- prok_raw$otu_table   
meta_raw  <- prok_raw$sample_table 
...
prok_microdata_full$cal_betadiv(unifrac = TRUE)
p_full <- trans_beta$new(dataset = prok_microdata_full, group = 'sample_type', 
                         measure = 'wei_unifrac')
...
prok_microdata_endo <- microtable$new(
  otu_table = abund_avg_final,
  sample_table = metadata_avg,
  tax_table = prok_raw$tax_table,
  phylo_tree = prok_raw$phylo_tree,
...
prok_microdata_endo$tidy_dataset()

# Save averaged abundances
prok_microdata_endo$cal_abund(rel = FALSE)
prok_microdata_endo$save_abund(dirpath = 'ENDO_prok_taxa_abund_raw_AVERAGED',
                              merge_all = TRUE, sep = '\t', quote = FALSE)
prok_microdata_endo$cal_abund()
prok_microdata_endo$save_abund(dirpath = 'ENDO_prok_taxa_abund_AVERAGED',
                              merge_all = TRUE, sep = '\t', quote = FALSE)

# Beta diversity and PCoA
prok_microdata_endo$cal_betadiv(unifrac = TRUE)
p_endo <- trans_beta$new(dataset = prok_microdata_endo, group = 'sample_site', 
                         measure = 'wei_unifrac')
