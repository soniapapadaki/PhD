...
prok_microdata$save_abund(dirpath = 'ENDO_prok_taxa_abund_raw_AVERAGED',
                          merge_all = TRUE, sep = '\t', quote = FALSE)

prok_microdata$cal_abund()
prok_microdata$save_abund(dirpath = 'ENDO_prok_taxa_abund_AVERAGED',
                          merge_all = TRUE, sep = '\t', quote = FALSE)
...
p3$cal_ordination(method = 'PCoA') # skipping anosim, permanova etc 
...
write.csv(envfit_table_pp, "endo_prok_envfit_results.csv", row.names = FALSE)
