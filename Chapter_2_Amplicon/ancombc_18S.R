...
euk@sam_data[["sample_type"]] = factor(euk@sam_data[["sample_type"]], 
...
set.seed(123)
output_f = ancombc2(data = euk, tax_level = "Family",
...
df_fig_type_filtered_f = df_fig_type_f %>%
  ...
  dplyr::filter(!grepl('Ochromonadales', taxon) &
                  !grepl('MAST-12C', taxon) &
                  !grepl('Labyrinthulomycetes', taxon) &
                  !grepl('Blastocladiales', taxon))
...
output_g = ancombc2(data = euk, tax_level = "Genus",
                  fix_formula = "sample_type",
                  rand_formula = NULL,
                  p_adj_method = "holm", pseudo_sens = TRUE,
                  prv_cut = 0.1, lib_cut = 1000, s0_perc = 0.05,
...
plot_height_inches_f <- n_taxa_f * 0.33   # 33 px per bar at 100 dpi
...
plot_height_inches_g <- n_taxa_g * 0.33   
