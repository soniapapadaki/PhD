...
abund_raw <- euk_raw$otu_table    # ASV x sample
meta_raw  <- euk_raw$sample_table # includes "sample_type_site"

### --------------------------
### FILTER OUT SOIL CRUSTS
### --------------------------
endolith_samples <- meta_raw$sample_type == "Endolith"

meta_raw  <- meta_raw[endolith_samples, ]
abund_raw <- abund_raw[ , rownames(meta_raw) ]
...
euk_microdata$save_abund(dirpath = 'ENDO_euk_taxa_abund_raw_AVERAGED',
                          merge_all = TRUE, sep = '\t', quote = FALSE)

euk_microdata$cal_abund()
euk_microdata$save_abund(dirpath = 'ENDO_euk_taxa_abund_AVERAGED',
                          merge_all = TRUE, sep = '\t', quote = FALSE)
...
# 18S PCoA using averaged microeco object
p3 <- trans_beta$new(
  dataset = euk_microdata,
  group = 'sample_site',        # your grouping variable
  measure = 'wei_unifrac'
)
...
write.csv(envfit_table_pp, "endo_euk_envfit_results.csv", row.names = FALSE)
