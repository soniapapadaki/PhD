...
set.seed(123)
output = ancombc2(data = prok, tax_level = "Family",
...
# Panel width in inches (0.65 in per x unit)
x_range <- 2.5 - (-3.5)
panel_width_inches <- x_range * 0.65

# Plot height in inches (0.32 in per taxon)
n_taxa <- length(unique(df_fig_type_filtered$taxon))
plot_height_inches <- n_taxa * 0.32

# Convert plot to grob and set panel width
